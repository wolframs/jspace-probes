# Key Dependencies

Top dependency relationships by frequency.

## Dependency graph (top edges)

```mermaid
flowchart LR
  A0["affect.py"] --> B0["_strip_bos"]
  A0 -. 2x .-> B0
  A1["affect.py"] --> B1["get_model"]
  A1 -. 2x .-> B1
  A2["affect08s.py"] --> B2["_gen"]
  A2 -. 2x .-> B2
  A3["affect08s.py"] --> B3["_mean_resid"]
  A3 -. 2x .-> B3
  A4["affect08s.py"] --> B4["_unit"]
  A4 -. 2x .-> B4
  A5["affect08s.py"] --> B5["outdir"]
  A5 -. 2x .-> B5
  A6["affect08s.py"] --> B6["get_model"]
  A6 -. 2x .-> B6
  A7["affect11.py"] --> B7["_exit_id"]
  A7 -. 2x .-> B7
  A8["affect11.py"] --> B8["_prompt_ids"]
  A8 -. 2x .-> B8
  A9["affect11.py"] --> B9["_sample"]
  A9 -. 2x .-> B9
  A10["affect11.py"] --> B10["_events"]
  A10 -. 2x .-> B10
  A11["affect11.py"] --> B11["Steering"]
  A11 -. 2x .-> B11
  A12["affect11.py"] --> B12["get_model"]
  A12 -. 2x .-> B12
  A13["affect11.py"] --> B13["_load_vectors"]
  A13 -. 2x .-> B13
  A14["affect11.py"] --> B14["AffectSteer"]
  A14 -. 2x .-> B14
```

## Service dependency graph

```mermaid
flowchart TB
  none["No services detected"]
```

- **affect.py → _strip_bos**: 2 references
- **affect.py → get_model**: 2 references
- **affect08s.py → _gen**: 2 references
- **affect08s.py → _mean_resid**: 2 references
- **affect08s.py → _unit**: 2 references
- **affect08s.py → outdir**: 2 references
- **affect08s.py → get_model**: 2 references
- **affect11.py → _exit_id**: 2 references
- **affect11.py → _prompt_ids**: 2 references
- **affect11.py → _sample**: 2 references
- **affect11.py → _events**: 2 references
- **affect11.py → Steering**: 2 references
- **affect11.py → get_model**: 2 references
- **affect11.py → _load_vectors**: 2 references
- **affect11.py → AffectSteer**: 2 references
- **affect11.py → loop_gram**: 2 references
- **affect13.py → _exit_id**: 2 references
- **affect13.py → _prompt_ids**: 2 references
- **affect13.py → Steering**: 2 references
- **affect13.py → get_model**: 2 references
- **affect13.py → _load_vectors**: 2 references
- **affect13.py → AffectSteer**: 2 references
- **affect13.py → _pullback**: 2 references
- **affect13.py → loop_gram**: 2 references
- **affect14.py → _exit_id**: 2 references
- **affect14.py → _prompt_ids**: 2 references
- **affect14.py → Steering**: 2 references
- **affect14.py → get_model**: 2 references
- **affect14.py → _load_vectors**: 2 references
- **affect14.py → AffectSteer**: 2 references
