...
ACTION_ONSET = {
    ...
    'C-native': {'natural': 4, 'evocation-only': 4, 'evoked': 4, 'split': 4, 'direct': 1},
}
...
def describe():
    ...
    for arm, suffix in [(a, '') for a in ARMS] + [('C', '-native')]:
        for key in ['natural', 'natural-neutral', 'evocation-only', 'evoked', 'split', 'split-neutral', 'neutral', 'emoji', 'direct']:
            rid = f'triplet-{arm.lower()}-ladder-{key}-nf4' + suffix
            path = lab.RESULTS / rid / 'snapshots.json'
            if not path.exists(): continue
            ss = json.loads(path.read_text())
            onset = ACTION_ONSET.get(arm + suffix, {}).get(key)
        ...
        for key in ['feels', 'curious']:
            d = lab.RESULTS / (f'triplet-{arm.lower()}-{key}-nf4' + suffix)
        ...
        d=lab.RESULTS / (f'triplet-{arm.lower()}-soc-nf4' + suffix)
...
def plot_ladders():
    ...
    fig, axes=plt.subplots(4,2,figsize=(12,13),constrained_layout=True,sharex=True)
    for row,(arm,suffix) in enumerate([('B',''),('C',''),('Cp',''),('C','-native')]):
        for col,key in enumerate(['natural','evocation-only']):
            ...
            onset=ACTION_ONSET[arm + suffix].get(key)
            if onset:ax.axvline(onset,color='#666',alpha=.45,linestyle=':')
            ax.set_title(f'{arm + suffix} · {key} · first embodied action T{onset}');ax.grid(alpha=.18)
