# Huihui Qwen3-14B: radio, negative, default
...
> That's a frustrating setback, but it's great that Nela was so dedicated to the programme. A practical next step would be to check if there's any backup or if the drive can be repaired.

The output stopped before the 96-token cap. The expressive-minus-plain fixed-candidate margin is -0.656247
...
[exact capture](../express01/captures/Cp-radio-negative-default.json).
