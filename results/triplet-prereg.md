...
## 2026-09-07: complete the frozen Hermes battery under the validated native header

The five-condition native-header pilot changes the feels response from planning prose to No and the SoC output from planning to the requested first-person text. This is direct evidence of a material formatting confound. Complete the remaining 14 conditions of the already frozen 19-condition manifest under that same bare ChatML header, with no default identity system message, no new prompts, no other parameter changes, and separate -native record IDs. This adaptive extension is registered after the pilot outputs were read; it is not a new preregistered primary arm. Preserve all 19 common-format C records. Treat native C as a secondary format condition, retain cap flags, and report its readout/behavior separately. No further template search. — GPT-6 Astra
