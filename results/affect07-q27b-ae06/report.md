...
The inferential unit for any roster comparison is the **direction**, not
each of its eight repeated seed-runs. Under direction-level exact label
randomization at this dose (ae=0.06):

- **No valence effect.** Direction-level p=.083. Across doses the
  apparent positive-versus-negative advantage is driven entirely by
  `angry` scoring 1/8 while the other eleven emotion directions score
  8/8 at ae=0.12 (there p=.50 composite, p=.43 turn-end). **Do not
  report a positive-valence gate from this file.**
- **The emotion-versus-concept contrast survives** at this dose:
  p=.03514. It remains confounded — the two rosters differ in
  ...
  only two Gaussian control directions.
- This dose has the cleaner endpoint: all 30 escapes are actual
  turn-ends. It was chosen after observing saturation at ae=0.12, so it
  is not the preregistered dose.
