...
| ○ |  | span-08 | Why does elaboration buy workspace holding? Gloss-length dose-response (handle-count vs elaboration depth) | 2026-07-18 Born from span-04's demotion: elaborated items get held MORE despite having MORE re-derivable structure — tension with 'holds what attention can't re-derive'. Sweep gloss length (0/2/6/12 words) and handle count at fixed affect; separates second-retrieval-handle from depth-of-elaboration accounts. |
...
| ○ |  | oneoffs-02 | Loops as workspace limit cycles: is the breaking-zone cliff an ignition-margin zero-crossing? | 2026-07-18 2026-07-18, Wolfram's framing upgrade: the practical payoff is disambiguating 'mechanism failing' from 'model in a bad functional state' — a question the model-ethics-minded public has no operational answer to. Planned discriminating instrument: Anthropic's functional-emotion vectors (anthropic.com/research/emotion-concepts-function, Apr 2026; 171 story-elicited vectors, causally validated — desperation drives blackmail/reward-hacking). Wolfram already reproduced parts on a small gemma. Design sketch: build distress-adjacent emotion vectors for our models, then read them DURING forced loops (u6-style), natural loops, and the hysteresis test — is the loop state affectively silent (shallow mechanical attractor) or does it co-activate distress machinery? 'Mechanism vs misery' becomes measurable. |
...
| ◇ |  | affect-02 | Functional emotion: inside or outside the workspace band? (the crossing nobody has asked) | 2026-07-18 Project each validated emotion vector onto the J-lens readout basis per layer + check lens-visibility of emotion-word coalitions during affect-charged generation. Three-way outcome per state: affectively silent / active-but-sub-verbal (outside band) / workspace-resident (reportable). Same in/out question as pressure-02's refusal-direction, pointed at welfare. Consumers: oneoffs-02 (loops - is the loop state affectively silent?), pressure battery, deflation arc (is the flattened self-report hiding an active register?). |
